...
class Invoice
{
     ...
     * @param bool $isDate
     */
    private function validateItem($name, $value, $maxLength = false,
                                  $isNumeric = false, $isDate = false)
    {
        try {
            if ($maxLength) Validators::assertMaxLength($value, $maxLength);
            if ($isNumeric) Validators::assertNumeric($value);
            if ($isDate) Validators::assertDate($value);
        } catch (\InvalidArgumentException $e) {
            throw new InvoiceException($this->getId().": ".$name." - ".$e->getMessage(),
            0, $e);
        }
    }
    ...
    {
        $this->validateItem('text', $text, 240);
        $this->text = $text;
    }
    ...
    public function setPaymentType($value)
    {
        ...
        if (is_null($value) || !isset($payments[$value])) {
            throw new InvoiceException($this->getId().": payment type $value is not supported. Use one of these: ".explode(",",
                array_keys($payments)));
        }
    }
    ...
    {
        $this->validateItem('contract', $contract, 19);
        $this->contract = $contract;
    }

    public function setCentre($centre)
    {
        $this->validateItem('centre', $centre, 19);
        $this->centre = $centre;
    }

    public function setActivity($activity)
    {
        $this->validateItem('activity', $activity, 19);
     ...
     */
    public function createCustomerAddress(array $customerAddress,
                                          $identity = null,
                                          array $shippingAddress = [])
    {
        try {
            ...
            throw new InvoiceException($this->getId().": ".$e->getMessage(), 0,
            $e);
        }
    }
    ...
    public function export(SimpleXMLElement $xml)
    {
        ...
        $this->exportHeader($xmlInvoice->addChild("inv:invoiceHeader", null,
                self::NS));
        if (!empty($this->items)) {
            $this->exportDetail($xmlInvoice->addChild("inv:invoiceDetail", null,
                    self::NS));
        }
        $this->exportSummary($xmlInvoice->addChild("inv:invoiceSummary", null,
                self::NS));
    }
    ...
    private function exportHeader(SimpleXMLElement $header)
    {
        ...
        if ($this->withVAT) {
            //tuzemske plneni
            $classification->addChild('typ:classificationVATType', 'inland',
                Export::NS_TYPE);
        } else {
            //nezahrnovat do dph
            $classification->addChild('typ:ids', 'UN', Export::NS_TYPE);
            $classification->addChild('typ:classificationVATType', 'nonSubsume',
                Export::NS_TYPE);
        }
        ...
        $paymentType->addChild('typ:paymentType', $this->paymentType,
            Export::NS_TYPE);
        if (!is_null($this->paymentTypeString)) {
            $paymentType->addChild('typ:ids', $this->paymentTypeString,
                Export::NS_TYPE);
        }
        ...
        $header->addChild("inv:intNote",
            'Tento doklad byl vytvořen importem přes XML.');
    ...
    private function exportDetail(SimpleXMLElement $detail)
    {
        foreach ($this->items AS $product) {
            ...
            $item->addChild("inv:payVAT",
                $product->isPayVAT() ? 'true' : 'false');
            if ($product->getRateVAT())
                    $item->addChild("inv:rateVAT", $product->getRateVAT());
            if ($product->getDiscountPercentage())
                    $item->addChild("inv:discountPercentage",
                    $product->getDiscountPercentage());

            if (!empty($product->getHomeCurrency())) {
                $hc = $item->addChild("inv:homeCurrency");
                if ($product->getUnitPrice())
                        $hc->addChild("typ:unitPrice", $product->getUnitPrice(),
                        Export::NS_TYPE);
                if ($product->getPrice())
                        $hc->addChild("typ:price", $product->getPrice(),
                        Export::NS_TYPE);
                if ($product->getPriceVAT())
                        $hc->addChild("typ:priceVAT", $product->getPriceVAT(),
                        Export::NS_TYPE);
            }
            ...
            if ($product->getStockItem()) {
                ...
                $stockItem = $stock->addChild("typ:stockItem", null,
                    Export::NS_TYPE);
                $stockItem->addChild("typ:ids", $product->getStockItem(),
                    Export::NS_TYPE);
            }
        }
    }
                                   ...
                                   $type = "address")
    {
        ...
        if (isset($data['country'])) {
            $address->addChild('typ:country')->addChild('typ:ids',
                $data['country']);
        }
    ...
    private function exportSummary(SimpleXMLElement $summary)
    {
        ...
        if (is_null($this->priceLowVAT) === false)
                $hc->addChild('typ:priceLowVAT', $this->priceLowVAT,
                Export::NS_TYPE); //dph ve snizene sazbe
        if (is_null($this->priceLowSum) === false)
                $hc->addChild('typ:priceLowSum', $this->priceLowSum,
                Export::NS_TYPE); //s dph ve snizene sazbe
        ...
        if (is_null($this->priceHightVAT) === false)
                $hc->addChild('typ:priceHighVAT', $this->priceHightVAT,
                Export::NS_TYPE);
        if (is_null($this->priceHighSum) === false)
                $hc->addChild('typ:priceHighSum', $this->priceHighSum,
                Export::NS_TYPE);
