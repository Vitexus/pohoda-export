...
class Address implements IExport
{
    ...
    public function export(SimpleXMLElement $xml)
    {
        ...
        $this->exportHeader($xmlAd->addChild('adb:addressbookHeader', null,
                self::NS));
    ...
    public function exportFilter(SimpleXMLElement $xml)
    {
        ...
        if ($this->getIdentity()->hasId()) {
            ...
            $ext->addChild('typ:ids', $this->getIdentity()->getId(),
                Export::NS_TYPE);
    ...
    public function exportAddress(SimpleXMLElement $xml)
    {
        ...
        if ($identity->hasShippingAddress())
                $this->exportAddressXml($xml, $identity->getShippingAddress(),
                self::SHIPTO);
    }

    public function exportAddressXml(SimpleXMLElement $xml,
                                     \Pohoda\Object\Address $address,
                                     $type = self::ADDRESS)
    {
        ...
        if ($address->getCountry()) {
            $xmlAd->addChild('typ:country')->addChild('typ:ids',
                $address->getCountry());
        }
